--  REST API Validation
--  API to validate
--
--  The version of the OpenAPI document: 1.0.0
--  Contact: Stephane.Carrez@gmail.com
--
--  NOTE: This package is auto generated by OpenAPI-Generator 7.11.0-2024-11-24.
--  https://openapi-generator.tech
--  Do not edit the class manually.

pragma Warnings (Off, "*is not referenced");
with OpenAPI.Streams;
package body Enums.Clients is
   pragma Style_Checks ("-bmrIu");

   Media_List_1 : constant OpenAPI.Mime_List :=
     (1 => OpenAPI.Mime_Json,
2       => OpenAPI.Mime_Xml);

   --  Get some stat from external struct
   procedure Do_Get_Enums
     (Client : in out Client_Type;
      Status : in     Enums.Models.Status_Type;
      Result :    out Enums.Models.Stat_Type_Vectors.Vector)
   is
      URI   : OpenAPI.Clients.URI_Type;
      Reply : OpenAPI.Value_Type;
   begin
      Client.Set_Accept (Media_List_1);

      URI.Set_Path ("/enums/{status}");
      URI.Set_Path_Param ("status", Enums.Models.To_String (Status));
      Client.Call (OpenAPI.Clients.GET, URI, Reply);
      Enums.Models.Deserialize (Reply, "", Result);
   end Do_Get_Enums;

   --  Get an object
   --  Get an object
   procedure Do_Get_Object
     (Client : in out Client_Type;
      Status : in     Enums.Models.Status_Type;
      Owner  : in     OpenAPI.Nullable_UString;
      Result :    out OpenAPI.Object)
   is
      URI   : OpenAPI.Clients.URI_Type;
      Reply : OpenAPI.Value_Type;
   begin
      Client.Set_Accept (Media_List_1);

      URI.Add_Param ("status", Enums.Models.To_String (Status));
      URI.Add_Param ("owner", Owner);
      URI.Set_Path ("/object");
      Client.Call (OpenAPI.Clients.GET, URI, Reply);
      OpenAPI.Streams.Deserialize (Reply, "", Result);
   end Do_Get_Object;
end Enums.Clients;
